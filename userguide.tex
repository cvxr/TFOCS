...
% TODO: add in proj)simplex, proj_psdUTrace.m; also add in the l1 + non-neg
%\begin{center}
    \begin{table}
        \centering
...
%    $h(y)=\iota_{\R^n_+}$ & \verb@proj_simplex@ & $h^*(z)=h(z)$ & \verb@proj_Rplus@ \\
%    $h(Y)=\iota_{Y \succeq 0}$ & \verb@proj_psdUTrace@ & $h^*(Z)=h(Z)$ & \verb@proj_psd@ \\
